...
CONFIGURE_OPTIONS = '--enable-coroutine_php'
SHLIB_SUFFIX_NAME = dylib
SHLIB_DL_SUFFIX_NAME = so
ZEND_EXT_TYPE = zend_extension
RE2C = exit 0;
AWK = awk
shared_objects_coroutine_php = coroutine_php.lo
PHP_PECL_EXTENSION = coroutine_php
PHP_MODULES = $(phplibdir)/coroutine_php.la
PHP_ZEND_EX =
...
./coroutine_php.la: $(shared_objects_coroutine_php) $(COROUTINE_PHP_SHARED_DEPENDENCIES)
	$(LIBTOOL) --mode=link $(CC) $(COMMON_FLAGS) $(CFLAGS_CLEAN) $(EXTRA_CFLAGS) $(LDFLAGS) -o $@ -export-dynamic -avoid-version -prefer-pic -module -rpath $(phplibdir) $(EXTRA_LDFLAGS) $(shared_objects_coroutine_php) $(COROUTINE_PHP_SHARED_LIBADD)
